...
/**
 * stylelintIgnore 设置忽略文件或目录
 * @param {object} file 
 * @param {string} conf 
 */
function stylelintIgnore(file, conf) {
...
module.exports = function(content, file, conf) {
    ...
    lintConfigs.ignoreFiles = makeArrItemUnique(lintConfigs.ignoreFiles);
    // 跳过忽略文件
    if (stylelintIgnore(file, lintConfigs)) {
        return;
    }
    ...
    stylelint.lint(lintConfigs).then(function(res) {
        ...
        if (result.warnings.length) {
            let formatResult = formatter(result);
            fis.log.warn('%s \n%s', file.id, formatResult.msgText);
            if (formatResult.errCount > 0) {
                process.exit();
            }
            return;
        }
